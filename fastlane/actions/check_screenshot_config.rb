  ...
  module Actions
    ...
    class CheckScreenshotConfigAction < Action
      def self.run(params)
        ...
        case platform
        when :android
          resolution = ("375x667,1080x810").split(",") if resolution.length == 0
        when :ios
          resolution = ("430x932x3,428x926x3,414x736x3,1024x1366x2").split(",") if resolution.length == 0
        else
          ## No default value
          UI.important "No default value for screenshoit resolution. Ignoring."
        end
